...
class SprintCommand(commands.Cog, CommandWrapper):
	...
	async def sprint(self, context, cmd=None, opt1=None, opt2=None, opt3=None):
		...
		**Sprint Tips**
		...
		args = await self.check_arguments(
			context, cmd=cmd, opt1=opt1, opt2=opt2, opt3=opt3
		)
		if not args:
			return
	...
	async def run_project(self, context, shortname):
		...
		if not project:
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("project:err:noexists", user.get_guild()).format(
					shortname
				)
			)
	...
	async def run_end(self, context):
		...
		if (
			int(sprint.get_createdby()) != user.get_id()
			...
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("sprint:err:cannotend", user.get_guild())
			)
	...
	async def run_declare(self, context, amount=None):
		...
		if new_amount < int(user_sprint["starting_wc"]) and not calculation:
			...
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("sprint:err:wclessthanstart", user.get_guild()).format(
					new_amount, user_sprint["starting_wc"], diff
				)
			)
		...
		await context.send(
			user.get_mention()
			+ ", "
			+ lib.get_string("sprint:declared", user.get_guild()).format(
				wordcount, written
			)
		)
	...
	async def run_status(self, context):
		...
		return await context.send(
			user.get_mention()
			+ ", "
			+ lib.get_string("sprint:status", user.get_guild()).format(
				current, written, elapsed, wpm, left
			)
		)
	...
	async def run_pb(self, context):
		...
		if record is None:
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("sprint:pb:none", user.get_guild())
			)
		else:
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("sprint:pb", user.get_guild()).format(int(record))
			)
	...
	async def run_leave(self, context):
		...
		await context.send(
			user.get_mention() + ", " + lib.get_string("sprint:leave", user.get_guild())
		)
		...
		if sprint.is_user_sprinting(user.get_id()):
			...
			if sprint_type == Sprint.SPRINT_TYPE_NO_WORDCOUNT:
				...
				await context.send(
					user.get_mention()
					+ ", "
					+ lib.get_string("sprint:join:update", user.get_guild()).format(
						starting_wc
					)
				)
			...
			if sprint_type == Sprint.SPRINT_TYPE_NO_WORDCOUNT:
				...
				await context.send(
					user.get_mention()
					+ ", "
					+ lib.get_string("sprint:join", user.get_guild()).format(
						starting_wc
					)
				)
	...
	async def run_time(self, context):
		...
		if not sprint.has_started():
			...
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("sprint:startsin", user.get_guild()).format(
					left["m"], left["s"]
				)
			)
		...
		# If it's currently still running, display how long is left
			...
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("sprint:timeleft", user.get_guild()).format(
					left["m"], left["s"]
				)
			)
		...
		# If it's finished but not yet marked as completed, we must be waiting for word counts
			...
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("sprint:waitingforwc", user.get_guild())
			)
	...
	async def run_notify(self, context):
		...
		return await context.send(
			user.get_mention()
			+ ", "
			+ lib.get_string("sprint:notified", user.get_guild())
		)
	...
	async def run_forget(self, context):
		...
		return await context.send(
			user.get_mention()
			+ ", "
			+ lib.get_string("sprint:forgot", user.get_guild())
		)
	...
	async def run_start(self, context, length=None, start=None):
		...
		if sprint.exists():
			return await context.send(
				user.get_mention()
				+ ", "
				+ lib.get_string("sprint:err:alreadyexists", user.get_guild())
			)
		...
		if (
			length is None
			or lib.is_number(length) is False
			or lib.is_number(length) <= 0
			or lib.is_number(length) > self.MAX_LENGTH
		):
		...
		if (
			start is None
			or lib.is_number(start) is False
			or lib.is_number(start) < 0
			or lib.is_number(start) > self.MAX_DELAY
		):
